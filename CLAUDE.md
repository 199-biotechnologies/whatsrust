...
- `src/api.rs` — REST API server (37 endpoints) + SSE streaming + CLI HTTP client
- `src/mcp.rs` — MCP server (15 tools, JSON-RPC over stdio, proxies to HTTP daemon)
...
- `src/main.rs` — binary: daemon mode (REPL + API) + CLI client (38 commands) + MCP mode
...
- Schema migrations via version check in `Store::new()` (currently v6)
